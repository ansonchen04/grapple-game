...
## Step 3: Screen-Space Input Projection (Quadrant-Safe Tangents)
- **What:** Replace fixed 90-degree tangent rotation with direct screen-space input projection. Calculate horizontal input as `Vector2(inputX, 0)`, then project it onto the rope's tangent plane by subtracting its radial component: `tangentialInput = inputDir - (inputDir.Dot(radialDir) * radialDir)`. This automatically aligns force with the correct arc direction regardless of quadrant.
- **Why:** Fixed vector rotations flip direction when crossing vertical/horizontal axes in Y-down coordinates, causing controls to reverse below the anchor. Projecting raw horizontal input onto the tangent plane guarantees "Left" always pushes left along the curve and "Right" always pushes right, naturally preserving momentum direction without manual quadrant checks or pole clamping.
- **Expected Result:** Controls remain consistent across all swing positions (above, below, or beside the anchor). Holding Left while swinging from bottom-left will continuously accelerate you leftward along the arc. No more input reversal or erratic snapping at vertical extremes.
...
## Step 11: Remove Tangential Speed Cap & Acceleration Falloff During Swings
- **Diagnosis:** The current `accelFalloff` calculation clamps input acceleration to zero when tangential speed exceeds `maxTangentialSpeed`. This acts as an artificial brake, preventing players from maintaining or building momentum while holding directional keys during high-speed swings. When moving faster than the cap, holding Left/Right adds no force, causing perceived slowdowns as natural physics (gravity/spring damping) take over unopposed.
...
## Step 13: Eliminate Jerkiness & Teleportation on Moving Platforms
- **Diagnosis:** The current system exhibits "jerky" or "teleporty" behavior when the rope stretches significantly on moving platforms due to three factors: (1) a hard position clamp (`GlobalPosition = anchor + radialDir * maxLen`) that instantly snaps the player, bypassing Godot's interpolation; (2) spring forces fighting against the platform's translation because anchor velocity isn't accounted for; and (3) unconditional radial velocity stripping that brakes legitimate momentum when the platform carries the player outward.
- **Fix 1: Soft Position Correction:** Remove direct `GlobalPosition` overrides. Instead, rely on increased spring stiffness at extreme stretches or gradually blend the distance toward the limit using force-based correction, allowing `MoveAndSlide()` to handle collisions smoothly.
- **Fix 2: Anchor Velocity Compensation:** Track `previousAnchor` each frame and compute `anchorVelocity = currentAnchor - previousAnchor`. Subtract this from the player's velocity before calculating radial/tangential components so the constraint only resists *relative* stretching, not platform translation.
- **Fix 3: Conditional Radial Damping:** Modify radial stripping to only dampen when `radialSpeed > 0` (moving outward relative to the anchor). Preserve inward or platform-carried momentum to prevent artificial braking.
